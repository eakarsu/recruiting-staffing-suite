const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4560';
...
const adminCookie = await login('admin@recruiting-staffing.local', 'admin123');
const managerCookie = await login('manager@recruiting-staffing.local', 'manager123');
const analystCookie = await login('analyst@recruiting-staffing.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/candidates', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/candidates', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/candidates/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/candidates/approve`, {
  method: 'POST',
  headers: {
